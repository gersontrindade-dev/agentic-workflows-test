...
6. When a meaningful update is ready, use the `create-pull-request` safe output exactly once to propose the change for Mona to review. Include a concise title and body summarizing the updates and citing the source URLs. Do not write directly to the default branch and do not push manually.